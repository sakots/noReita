...
- GithubAPI経由でのneo取得をデフォルトではオフに
- デバッグモードを利用できるIPを指定する機能追加

### [2026/09/19] v4.12.0

- configにデバッグモードスイッチを追加
- 画像がないときにはalt入力欄を表示しないようにした

### [2026/09/17] v4.11.2

- GitHub APIのNEO取得に5秒のタイムアウトを追加

### [2026/09/16] v4.11.1

- アプレットフィットが動作しなくなっていたのを修正

### [2026/09/16] v4.11.0

- neoをGithubAPI経由で直接最新版を読み込むようにした
  - 何らかの理由でGithubAPIを読み込めない場合は、従来のURLから読み込みます
...
- 管理画面で非表示にしたスレッドにもリプライできるのを修正
- NSFW投稿のサムネイル名が空文字・NULLの場合にOGPでぼかしが外れる問題を修正
- 極端な縦横比をしたサイズの画像をリサイズすると例外が発生するのを修正
- 不正な文字コードで禁止ワードチェックをすり抜けられる問題を修正
- 禁止ワードしきい値がオーバーフローすることがあるのを修正
- 禁止ワードしきい値が0でもカウントされていたのを修正
- リプライに失敗してもスレッドのageが実行されるのを修正
- 不正なスレッドへの返信で削除できないリプライが発生する問題を修正
- 外部から不正に「そうだね」できる問題を修正
- 画像の幅・高さを先に検証し、上限内の画像だけGDで展開するようにした
- DB保存成功まで画像などを一時保持し、失敗時は今回公開したファイルとサムネイルを削除して再投稿可能な状態を維持するようにした
- 画像差し替え時にPSDが保存対象から漏れ、更新したレイヤーデータがテンポラリに残るのを修正
- 画像差し替え後も、DBの幅・高さが古いままだったのを修正
- LitaChixなど再生対象外のツールにも動画再生リンクが出ていたの修正
- 画像差し替えでツールを変更した場合に使用ツールの記録が更新されないのを修正
- 描画開始時刻が未来の場合に発生する問題を修正
- 動画アップロード時に画像サイズの上限確認より先にGDで画像を展開していたのを修正
- 描画画像の容量上限を増やしても、投稿確定・差し替え時は10MB固定で拒否されていたのを修正
- 存在しない動画から続きが描ける可能性があるのを修正
- NSFW切り替え時はDB更新成功まで旧サムネイルを保持するようにした
- 投稿編集と「そうだね」の加算が重なると、件数が古い値に戻る可能性があったのを修正
- 保存直前に投稿が削除されるなど編集のDB更新が0件でも成功扱いになっていたのを修正
- 「そろそろ消える」スレッドが増加し続けるのを修正

### [2026/09/04] v4.7.0

- 直接アップロード画像は、公開前に必ずGDで再エンコードするようにした
  - これにより不要なexif情報を削除できます
- 親投稿を削除すると、レスを含むスレッド全体を削除するようにした
  - 各レスの画像・PCH・PSD・サムネイルも削除されます
- 最大スレッド数超過時の自動削除でも、残されたレスの画像等を削除するようにした
...
- 画像をクリップボードにコピーするリンクを追加
- 画像を差し替えると「そうだね」がリセットされていたのを修正

### [2026/09/03] v4.5.1

- AVIFの任意バイト列から巨大な画像寸法を取得してメモリが枯渇する問題を修正
- 偽装アニメーションwebpによる脆弱性を修正

### [2026/09/02] v4.5.0

- トリップのプレビュー機能追加
- アニメーションpngをNSFWでアップロードした際にアニメーションしなくなっていたのを修正
- アニメーションAVIFをNSFWでアップロードした際にぼかしをかけられなかったのでcssで代替
- アニメーションwebpがアップロードできなかったのを修正

### [2026/09/01] v4.4.0

- 外部リンクのOGP取得・キャッシュ・画像カードを表示するようにした
- 画像アップロードのプレビュー機能追加
