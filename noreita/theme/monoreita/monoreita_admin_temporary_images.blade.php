...
<body>
  ...
  <main>
    <section class="thread">
      ...
      <p>投稿前の画像を確認・削除できます。期限切れは {{$temporary_days}} 日を過ぎたファイルです。表示は新しい順に最大 {{$admin_temporary_images_limit}} 件です。</p>
      <form action="{{$self}}?mode=admin_temporary_images_manage" method="post">
        ...
        @if (empty($admin_temporary_images))
          <p>管理対象の一時画像はありません。</p>
        @else
          <p>{{$admin_temporary_images_total}} 件中 {{count($admin_temporary_images)}} 件を表示しています。</p>
          <table class="delfo">
