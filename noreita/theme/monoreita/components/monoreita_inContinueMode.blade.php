    ...
    @foreach ($oya as $bbsline)
      ...
      <form action="{{$self}}" method="post" enctype="multipart/form-data">
        ...
        <select class="form" name="ctype">
          ...
          @if ($has_psd && !$ctype_pch)
            <option value="psd" class="klecks-psd-option" hidden>PSDから</option>
          @endif
        </select>
        ...
        @if ($passflag) Pass<input class="form" type="password" name="pwd" size="8" value="" autocomplete="off"> @endif
        @if (!$ctype_pch)
          <label for="tools">ツール</label>
          <select name="tools" class="continue-tools">
          ...
          @if ($has_psd)
            <script>
              (() => {
                ...
                const tools = form.querySelector('.continue-tools');
                const psd = form.querySelector('.klecks-psd-option');
                if (!tools || !psd) return;
                const updatePsdOption = () => {
                  psd.hidden = tools.value !== 'klecks';
                  if (psd.hidden && psd.selected) form.querySelector('[name="ctype"]').value = 'img';
                };
                tools.addEventListener('change', updatePsdOption);
                updatePsdOption();
              })();
            </script>
          @endif
