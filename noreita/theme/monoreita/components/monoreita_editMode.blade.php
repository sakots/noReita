    ...
    @foreach ($oya as $bbsline)
      <form class="ppost postform" action="{{$self}}?mode=editexec" method="post">
        <table>
          <tr>
            <td>name</td>
            <td><input type="text" name="name" size="28" autocomplete="section-edit username" value="{{$bbsline['input_name'] ?? $bbsline['a_name']}}" maxlength="{{$max_name}}" data-trip-preview-input><output data-trip-preview hidden aria-live="polite"></output></td>
          </tr>
          <tr>
            <td>mail</td>
          ...
          <tr>
            ...
            <td>
              ...
              @else
              <input type="password" name="pwd" size="8" value="{{$bbsline['input_password'] ?? $pwd_cookie}}" autocomplete="section-edit current-password" onkeydown="if(event.ctrlKey&&event.keyCode==13){document.getElementById('send').click();return false};">
              @endif
            </td>
          </tr>
          <tr>
