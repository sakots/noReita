...
投稿画像と返信画像は API のサムネイルを表示します。画像 URL は現在表示している
設置先の相対 URL に正規化するため、`site.base_url` のホスト名が異なる環境でも
同一オリジンから読み込まれます。

スレッドへのリンクも現在表示中の入口に対する `?resno=` を使用するため、
`index.php` を公開 URL に含めない設置でも動作します。
