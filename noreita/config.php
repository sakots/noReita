...
return [
  ...
  'spam' => [
    ...
    // 本文の拒否スコア規則です。各項目は[正規表現, 加点値]で指定します。
    // 複数の規則に一致した点数を合計し、comment_score_threshold以上なら投稿を拒否します。
    'comment_score_rules' => [],
    // 本文を拒否する合計スコアです。0ならスコア判定を無効にします。
    'comment_score_threshold' => 0,
  ...
  'security' => [
    ...
    // X-Forwarded-Forを信頼する直近のリバースプロキシです。通常の設置では空のままにします。
    // 必要な場合だけIPアドレスまたはCIDRを列挙してください。
    'trusted_proxies' => [],
  ...
  'paths' => [
    // 使用するテーマのディレクトリ名です。v4.2簡易テーマはtheme.phpでedaなどを継承できます。
    'theme' => 'eda',
  ...
  'limits' => [
    ...
    'image_height' => 4000,
    // 直接アップロード画像を保存時に縮小する最大幅です。0で幅による縮小を無効にします。
    'upload_resize_width' => 1920,
    // 直接アップロード画像を保存時に縮小する最大高さです。0で高さによる縮小を無効にします。
  ...
  // 開発時のエラー表示に関する設定です。本番環境では必ずfalseのままにしてください。
  'debug' => [
    // trueにすると、allowed_ipsに指定した接続元だけへエラー詳細とスタックトレースを表示します。
    'enabled' => false,
    // デバッグ表示を許可する接続元IPアドレスです。空の場合は誰にも表示しません。
    'allowed_ips' => [],
  ],

  // 管理操作の監査ログは、外部リクエストで増えるエラーログと別枠で保存します。
  'audit_log' => [
    // 監査ログを保存する日数です。
    'retention_days' => 365,
    // 1個の監査ログファイルの最大サイズです（バイト）。
    'max_bytes' => 5242880,
    // 1日に作成するローテーション済み監査ログファイルの最大数です。
    'max_files_per_day' => 5,
  ],
