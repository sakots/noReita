...
`AdminLoginRateLimiter`は管理者ログインの失敗回数を接続元IP単位で制限します。生IPや管理パスは保存せず、管理パスを鍵にしたIPのHMACと、回数・時刻だけを`session/`内の`0600`ファイルへ保存します。既定では15分以内に5回失敗すると15分間HTTP 429で拒否し、ログイン成功時に記録を削除します。転送ヘッダーを偽装して制限を回避されないよう、判定には`REMOTE_ADDR`だけを使用します。
