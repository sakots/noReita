...
管理者は`mode=admin_temporary_images`から、投稿前の一時画像を確認できます。選択削除は有効な`.dat`メタデータと一致する画像だけを対象にし、画像・動画・作業データを同じベース名でまとめて削除します。期限切れの一時ファイルは、設定された`limits.temporary_days`に従って一括整理できます。
